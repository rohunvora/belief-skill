...
export interface TradeExpression {
  ...
  // Research-validated display metrics
  conviction_breakeven_pct: number;   // "You need to be right X% for this to be +EV"
  probability_of_profit?: number;     // POP: single scannable % (Tastytrade pattern)
  platform_risk_tier: PlatformRiskTier; // Visible on every card
  correlation_group?: string;         // e.g., "rates_bearish" — trades sharing a group are correlated
